...
density_mod <- lmer(formula = SnailDensity~mean + (1|AggregationIndex), data = mean_contact)
summary(density_mod)
mean_contact$mean_predict <- predict(density_mod, type = "response")
#Linear model for density vs aggregation index
aggregation_mod <- lmer(formula = SnailDensity~AggregationIndex + (1|mean), data = mean_contact)
